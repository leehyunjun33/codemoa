...
const upload = multer({
  ...
  router.post('/creatLecture', function(req: any, res: any){
  ...
  router.get('/myLecture', function(req: any, res, next){
...
router.get('/user', function(req, res, next){
  ...
  router.get('/myChooseAnwser', function(req: any, res, next){
 ...
 router.post('/register', function(req:any, res){
    ...
    conn.query(sql, [email, password, name, grade,c_name,m_img], function(err:any, data:any){
       if(err){
           console.log("에러남", err);
           res.send(false); 
       }else{
 ...
 router.post('/setMemo', function(req:any, res){
  ...
  conn.query(sql, [id, l_id], function(err:any, data:any){
     if(err){
       ...
       console.log("메모데이터확인", data);
        if(data[0]){
          conn.query(sqlUpdate, [content, id, l_id], function(err:any, data:any){});
          console.log("메모업데이트");
        }else{
          conn.query(sqlInsert, [id, l_id, email, content], function(err:any, data:any){});
...
router.post('/getMemo', function(req:any, res){
  req.body = Object.keys(req.body);
  req.body = JSON.parse(req.body[0]);
  ...
  conn.query(sql, [m_id, l_id], function(err:any, data:any){
     if(err)
         console.log("에러남", err);
 ...
 router.post('/awnserInput', function(req:any, res){
    ...
    conn.query(sql, [q_id, q_name, a_name, content,email], function(err:any, data:any){
       if(err){
           console.log("에러남", err);
           res.send(false); 
       }else{
 ...
 router.get('/myBookmark', function(req:any, res){
...
router.post('/chooseAwnser', function(req:any, res){
  ...
  conn.query(sql, function(err:any, data:any){
     ...
     conn.query(sql1_question, function(err:any, data:any){});
     conn.query(sql2,email, function(err:any, data:any){
        if(err)
            console.log("채택 에러(멤버점수))", err);

        conn.query(sql3_select,email, function(err:any, data:any){
          var string = JSON.stringify(data)         
          var json = JSON.parse(string);
          var grade = 'F';
